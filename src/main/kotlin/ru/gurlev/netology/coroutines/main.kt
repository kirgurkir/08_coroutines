package ru.gurlev.netology.coroutines
...
import ru.gurlev.netology.coroutines.dto.*
                ...
                val posts = getPosts(client)
                    .map { post ->
                        async {
                            ...
                            val allComment: List<Pair<Comment, Author>> = getComments(client, post.id)
                                .map { comment ->
                                    async {
                                        Pair(comment, getAuthor(client, comment.authorId))
                                    }
                                }.awaitAll()

                            FullPost(post, allComment, getAuthor(client, post.authorId))
                        }
                    }.awaitAll()
